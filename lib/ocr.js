...
/**
 * Extrait les frames d'une vidéo à un FPS donné
 * @param {string} videoPath - Chemin vers la vidéo
 * @param {number} fps - Nombre de frames par seconde à extraire (défaut: 2)
 * @returns {Promise<string[]>} - Tableau des chemins des frames extraites
 */
function extractVideoFrames(videoPath, fps = 2) {
  return new Promise((resolve, reject) => {
    ...
    if (debug) console.log('[ocr] Extracting video frames to:', frameDir);
    ...
    ffmpeg(videoPath)
      ...
      .on('end', () => {
        ...
        if (debug) console.log('[ocr] Extracted', frames.length, 'frames');
        resolve(frames);
      })
      .on('error', (err) => {
