...
export function buildPayloadFromUrl(url, { rankMin = 'Bronze' } = {}) {
  const baseUrl = process.env.BONUS_BASE_URL || 'https://playstake.club/bonus';
  const buttonLabel = (process.env.BUTTON_LABEL_TEXT || '🎁 Lien du bonus').slice(0, 80);
  ...
  const rec = inferBonusRecord({ url, code }) || { kind: 'unknown', title: '🎁 Bonus Stake', intro: 'Bonus détecté :' };
  const title = renderTitle(rec.title);
  const intro = renderIntro(rec.intro, rankMin);
  const linkUrl = `${baseUrl}?code=${encodeURIComponent(code)}`;

  const embed = new EmbedBuilder()
    .setTitle(title)
    .setDescription(intro)
    .setColor(0x5865F2)
    .setFooter({ text: 'PlayStake - Official Mirror List for Stake' });
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setURL(linkUrl)
      .setLabel(buttonLabel) // <-- ici : “Lien du bonus”
  );
